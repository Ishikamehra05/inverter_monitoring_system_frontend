...
export default function EditOperatorModal({
  ...
  const normalizeTimezone = (value?: string) => {
    if (!value) return "UTC+05:30";

    // Already in required format
    if (/^UTC[+-]\d{2}:\d{2}$/.test(value)) {
      return value;
    }

    // Handle Asia/Kolkata if your API/database returns it
    if (value === "Asia/Kolkata") {
      return "UTC+05:30";
    }

    // Handle full display value
  ...
  const [formTimezone, setFormTimezone] = useState(normalizeTimezone(timezone));
            ...
            <div>
              <div className="relative">
                <input
                  ...
                  className={`w-full h-[38px] px-3 pr-10 border rounded-[4px] outline-none transition ${passwordError ? "border-[#ff4d4f]" : "border-[#d9d9d9]"
                    } focus:border-[#40a9ff]`}
                />
